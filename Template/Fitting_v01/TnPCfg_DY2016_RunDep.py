...
#############################################################################################
# -- Usage: cmsRun TnPCfg_DY2016_Final.py <scenario> <effType> [<systMode>]
# -- args[0]: scenario
# ---- possible scenario
# ------ data_25ns
# ------ mc_weight
# -- args[1]: efficiency type
# -- args[2]: systematic uncertainty estimation mode (optional)
# ---- possible modes:
# ------ altSignalShape
# ------ tagLoose
# ------ tagTight
...
# ------ massRangeUp
# ------ massRangeDown
# -- ex>  cmsRun TnPCfg_DY2016_Final.py data_25ns RelTrkIso_010_from_HighPt
# ---ex2> cmsRun TnPCfg_DY2016_Final.py data_25ns RelTrkIso_010_from_HighPt altSignalShape
#############################################################################################
...
# -- TnP parameters to be varied for systematic uncertainty estimation
# -- details on fit shpaes: can be adjusted in Determine_Shape
fitShape = ""
if PassingProbe == "HighPt": fitShape = "vpvPlusCMS"   # -- ID:   double voigtian + CMS shape
else:                        fitShape = "vpvPlusExpo3" # -- else: double voigtian + exponential
...
# -- systematic uncertainty estimation part
systMode = ""
if sys.argv[0] == "cmsRun" and len(args) > 2:
    systMode = args[2]
    print "sysetmatic uncertianty mode: ", systMode

    if systMode == "altSignalShape":
        # -- details on fit shpaes: can be adjusted in Determine_Shape
...
def Determine_Shape( _PassingProbe, _ProbeCondition ):
    _shape = cms.vstring( fitShape ) # -- default -- #
...
Template = cms.EDAnalyzer("TagProbeFitTreeAnalyzer",
    ...
    Variables = cms.PSet(
        mass = cms.vstring("Tag-muon Mass", "%.1lf" % minMass, "%.1lf" % maxMass, "GeV/c^{2}"),
        ...
        tag_combRelIsoPF04dBeta = cms.vstring("pf relative isolation (tag)", "0", "999", ""),
    ...
    binsForFit = cms.uint32(nMassBin),
...
PT_BINS = cms.PSet(
    pt = cms.vdouble( 17, 23, 28, 40, 50, 60, 120, 200, 500, 1000 ),
...
process.TnP_MuonID = Template.clone(
    ...
    OutputFileName = cms.string("TnP_DY2016_%s.root" % scenario),
...
if "_weight" in scenario:
    ...
    process.TnP_MuonID.Variables.weight = cms.vstring("weight","-100.0","100.0", "")
...
for ID in IDS:
    ...
    for X,B in ALLBINS:
        ...
        DEN.tag_pt = cms.vdouble(minTagPt, 9999)
        DEN.tag_abseta = cms.vdouble(0, 2.4)
        DEN.tag_IsoMu24 = cms.vstring("pass")
        DEN.tag_combRelIsoPF04dBeta = cms.vdouble(0, maxTagIso)
