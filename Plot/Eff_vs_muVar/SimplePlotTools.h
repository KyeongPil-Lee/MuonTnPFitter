...
public:
  ...
  // -- canvas size
  Bool_t setCanvasSize_ = kFALSE;
  Double_t canvasWidth_ = 0;
  Double_t canvasHeight_ = 0;
  ...
  void SetCanvasSize( Double_t width, Double_t height) {
...
public:
  ...
  {
    HistInfo histInfo{ (TH1D*)h->Clone(), legend, color, 20 }; // -- 20: circle
    histInfos_.push_back( histInfo );
  }

  void Register( TH1D* h, TString legend, Int_t color, Int_t marker)
  {
    HistInfo histInfo{ (TH1D*)h->Clone(), legend, color, marker };
    histInfos_.push_back( histInfo );
  }
  ...
  void Draw( TString drawOp = "EPSAME" )
  {
    ...
    SetCanvas_Square();
    if( setCanvasSize_ )
      c_->SetCanvasSize(canvasWidth_, canvasHeight_);
    ...
    for(Int_t i=0; i<nHist; i++)
    {
      ...
      Int_t marker = histInfos_[i].marker;
...
public:
  ...
  void Draw( TString drawOp = "EPSAME" )
  {
    ...
    for(Int_t i=0; i<nHistRatio; i++)
    {
      ...
      Int_t  marker   = histInfoRatios_[i].marker;
  ...
  void CalcRatioHist()
  {
    ...
    for(Int_t i=1; i<nHist; i++) // -- starts with 1 -- //
    {
      ...
      HistInfo histInfoRatio{ h_ratioTemp, legend, color, marker };
...
public:
  ...
  void Draw( TString drawOp = "EPSAME" )
  {
    ...
    Int_t markerRatio = histInfo_data_.marker;

    h_ratio_dataToStack_->Draw(drawOp);
    h_ratio_dataToStack_->SetStats(kFALSE);
    h_ratio_dataToStack_->SetMarkerStyle(markerRatio);
...
private:
  ...
  void SetDataHistogram(TLegend *legend)
  {
    ...
    Int_t marker        = histInfo_data_.marker;
...
public:
  ...
  {
    GraphInfo graphInfo{ (TGraphAsymmErrors*)g->Clone(), legend, color, 20 }; // -- 20: circle
    graphInfos_.push_back( graphInfo );
  }

  void Register( TGraphAsymmErrors* g, TString legend, Int_t color, Int_t marker)
  {
    GraphInfo graphInfo{ (TGraphAsymmErrors*)g->Clone(), legend, color, marker };
    graphInfos_.push_back( graphInfo );
  }
